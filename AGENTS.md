...
- **LLM**: `OPENAI_BASE_URL=https://opencode.ai/zen/go/v1` (verified against `/opt/omi/.env` 2026-08-25; `OPENROUTER_BASE_URL=https://9router.xuanloi.me/v1` stays as fallback) + model override via `OMI_MAIN_MODEL=mimo-v2.5` / `OMI_LIGHT_MODEL=mimo-v2.5` (patched in `backend/utils/llm/model_config.py` to read these env vars). Keep `selfhost/.env.template`, this file, and the live `/opt/omi/.env` in sync — they must never drift.
- **Embeddings**: `OMI_EMBEDDINGS_BASE_URL` / `OMI_EMBEDDINGS_API_KEY` / `OMI_EMBEDDINGS_MODEL=gemini-embedding-001` (3072-dim, Gemini free tier) route embeddings off the zen gateway, which has no `/v1/embeddings` (`backend/utils/llm/clients.py`, `OMI_EMBEDDINGS_*` block in `.env.template`). Zen does NOT serve embeddings.
- **STT routing (2026-08-26 benchmark)**: Vietnamese STT primary = Deepgram cloud `nova-3` `vi` (WER ~2.6%, punctuation built-in); local sherpa stays as fallback. Switch via `DEEPGRAM_SELF_HOSTED_ENABLED=false` on `/opt/omi/.env` → tokens route to `api.deepgram.com`. Benchmark data: `docs/superpowers/reports/2026-08-25-stt-benchmark-results.md`. NOTE `language=multi` on nova-3 is catastrophic for Vietnamese (WER 100%) — always resolve explicit `vi`.
- **Transcript punctuation**: LLM post-processing at finalization, env-gated `OMI_PUNCTUATE_ENABLED=true` (`backend/utils/conversations/punctuation.py`), fail-open.
- **Output language**: `OMI_FORCE_OUTPUT_LANGUAGE=vi` forces title/summary language regardless of app-reported `language_code` (`_effective_output_language` in `process_conversation.py`).
- **n8n**: STOPPED on the VPS (data volume kept). Restore = `docker compose up -d n8n` from `/opt/n8n`.
- **Modal GPU services**: deployed but stopped by default — start/stop per need to control billing; backend falls back to local ONNX paths when absent.
- **STT**: `DEEPGRAM_SELF_HOSTED_ENABLED=false` (current live: cloud primary `nova-3 vi` per benchmark; set `true` to fallback to local sherpa) + `DEEPGRAM_SELF_HOSTED_URL=http://stt-adapter:8092`. Flag value MUST be lowercase string `'true'` when enabled (code checks `== 'true'` at `streaming.py:581`). Adapter fallback uses VietASR offline 70k-hour Vietnamese model (`sherpa-onnx-zipformer-vi-int8-2025-04-20`) with Silero VAD segmentation.
