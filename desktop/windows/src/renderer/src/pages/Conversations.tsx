...
const TYPE_TABS: { value: FilterKind; labelKey: string; icon?: LucideIcon }[] = [
  { value: 'all', labelKey: 'conversations.typeTabs.all' },
  { value: 'chat', labelKey: 'conversations.typeTabs.chats', icon: MessageSquare },
  { value: 'recording', labelKey: 'conversations.typeTabs.recordings', icon: Radio }
]
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-2 px-6 pb-3 lg:px-10">
        ...
        <button
          ...
          title={t('conversations.select.title')}
        >
          <CheckSquare className="h-4 w-4" />
          <span className="hidden sm:inline">
            {selectMode ? t('conversations.select.done') : t('conversations.select.select')}
          </span>
        </button>
      </div>
          ...
          <div className="surface-panel mx-auto mb-5 flex max-w-5xl items-center justify-between gap-3 px-4 py-3">
            <span className="text-sm text-white/60">
              {backfillRunning && backfill
                ? t('conversations.backfill.syncing', {
                    done: backfill.synced + backfill.failed,
                    total: backfill.total
                  })
                : t('conversations.backfill.onlyOnDevice', {
                    count: unsyncedPast,
                    plural: unsyncedPast === 1 ? '' : 's',
                    verb: unsyncedPast === 1 ? 'is' : 'are'
                  })}
              {!backfillRunning && backfill?.capped && (
                <span className="text-white/40"> · {t('conversations.backfill.hourlyLimit')}</span>
              )}
            </span>
            {!backfillRunning && (
              <button
              ...
              >
                {t('conversations.backfill.action')}
              </button>
            )}
          </div>
        )}
          ...
          <EmptyState
            ...
            title={
              anyFilter
                ...
                : t('conversations.empty.noConversationsTitle')
            }
            description={
              anyFilter
                ? t('conversations.empty.noMatchDesc')
                : t('conversations.empty.noConversationsDesc')
            }
            action={
              anyFilter ? (
                <button onClick={clearAllFilters} className="btn-ghost">
                  {t('conversations.empty.clearFilters')}
                </button>
              ) : (
                <Link to="/home" className="btn-record">
                  <Mic className="h-4 w-4" />
                  {t('conversations.empty.startRecording')}
                </Link>
              )
            }
          />
      ...
      {pendingDelete && (
        <div className="glass-strong mx-6 mb-4 flex items-center justify-between rounded-2xl px-4 py-3 lg:mx-10">
          <span className="text-sm text-white/80">
            {t('conversations.undo.willDelete', {
              count: pendingDelete.ids.length,
              plural: pendingDelete.ids.length !== 1 ? 's' : ''
            })}
          </span>
          <button
            onClick={undoDelete}
